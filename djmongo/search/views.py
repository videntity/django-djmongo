...
from .forms import SavedSearchForm, DatabaseAccessControlForm, PublicReadAPIForm
...
def setup_get_httpauth(request, database_name, collection_name):
    name = _("Setup HTTP GET API using HTTPAuth")

    dac, created = DatabaseAccessControl.objects.get_or_create(
...
def setup_get_public(request, database_name, collection_name):
    name = _("Setup HTTP GET API with no authentication (Public)")
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            return HttpResponseRedirect(
                reverse(
                    'djmongo_browse_saved_searches_w_params',
                    args=(
                        database_name,
                        collection_name)))
        else:
